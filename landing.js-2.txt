...
ru:{
  ...
  s1t:'Данные для записи',
  s2t:'Класс ребёнка',
  s2hint:'Выберите класс — покажем состав комплекта.',
  s3t:'Выберите предмет',
  s3hint:'Тест откроется прямо здесь. Пройти можно все предметы по очереди — результат по каждому сохранится.',
  sumLabel:'Вы выбрали:',
...
kz:{
  ...
  s1t:'Жазылу деректері',
  s2t:'Баланың сыныбы',
  s2hint:'Сыныпты таңдаңыз — жинақтың құрамын көрсетеміз.',
  s3t:'Пәнді таңдаңыз',
  s3hint:'Тест осы жерде ашылады. Пәндерді кезекпен өтуге болады — әрқайсысының нәтижесі сақталады.',
...
(function(){
  ...
  /* Шаги открываются по очереди: карточка → класс → предмет.
     Пока предыдущий не закрыт, следующий приглушён и не кликается. */
  function updateSummary(){
    $('#s2').classList.toggle('is-off', !state.student);
    $('#s3').classList.toggle('is-off', !(state.student && state.grade));
    if(state.student && state.grade) renderSubjects();
  }
  ...
  document.addEventListener('click', e=>{
    ...
    setTimeout(()=> $('#s3').scrollIntoView({behavior:'smooth', block:'center'}), 80);
  });
  ...
  $('#form').addEventListener('submit', async e=>{
    ...
    // Карточка идёт первым шагом: класс на этот момент ещё не выбран,
    // поэтому в заявку он попадает как «не указан» и уточняется дальше.
    state.student = {name: name, phone: '+' + digits};
    const payload = {
    ...
    // Карточка принята — открываем выбор класса. Форму сворачиваем в строку,
    // чтобы было видно, на кого записались, но она не мешала.
    $('#s1').classList.add('is-done');
    $('#s1').innerHTML = `<div class="bstep-h"><span class="bstep-n">✓</span>` +
      `<span class="bstep-t">${esc(name)} · +${digits}</span></div>`;
    $('#s2').classList.remove('is-off');
    updateSummary();
