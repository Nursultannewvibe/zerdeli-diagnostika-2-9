...
(function () {
  ...
  var CONFIG = {
    ...
    // Адрес абсолютный и вшит сборкой: страница живёт на домене Webflow,
    // а тесты — на GitHub Pages, относительный путь вёл бы в никуда.
    base: (window.ZERDELI_TESTS_BASE || 'https://nursultannewvibe.github.io/zerdeli-diagnostika-2-9/'),
  ...
  var T = {
    ru: {
      ...
      back: 'К списку предметов', zoom: 'Нажмите на картинку, чтобы увеличить',
    ...
    kz: {
      ...
      back: 'Пәндер тізіміне', zoom: 'Үлкейту үшін суретті басыңыз',
  ...
  var t = T[lang];
  // Контейнер ищем каждый раз заново, а не один раз при загрузке.
  // Лендинг создаёт #zd-test своим скриптом, и кто отработает первым —
  // не наше дело: при загрузке движка этого div'а может ещё не быть.
  function root() { return document.getElementById('zd-test'); }
  ...
  // Картинку во весь экран: закрывается тапом или Esc, масштаб — жестом.
  function lupa(src) {
    var box = document.createElement('div');
    box.className = 'zd-lupa';
    box.innerHTML = '<img src="' + esc(src) + '" alt="">';
    function zakryt() {
      box.remove();
      document.removeEventListener('keydown', poKlavishe);
    }
    function poKlavishe(e) { if (e.key === 'Escape') zakryt(); }
    box.onclick = zakryt;
  ...
  function openTest(id) {
    ...
    loadBank().then(function (bank) {
      ...
      // Вторая линия обороны. Лендинг такой тест не даёт открыть, но если
      // сюда всё же зашли (прямой ссылкой ?test=…), считать нельзя:
      // без правильных ответов любой ребёнок получит 0%.
      if (!data.answersFilled) { data.zdNotReady = true; return data; }
      return loadImages(data).then(function () { return data; });
    }).then(function (data) {
      if (data.zdNotReady) {
        el('<div class="zd-card"><h1 class="zd-h1">' + esc(t.notReady) + '</h1>' +
          '<p class="zd-sub">' + esc(t.notReadyNote) + '</p>' +
          '<a class="zd-btn" href="https://wa.me/' + CONFIG.whatsapp + '">' + esc(t.cta) + '</a>' +
      ...
      if (hooks.gate || state.student) {
        state.i = 0; state.answers = {}; state.started = Date.now();
        save();
    ...
    el('<div class="zd-card">' +
      ...
      (q.blockImage
        ? [].concat(q.blockImage).map(function (src) {
            // Исходники сняты широкими полосами (бывает 1200 px на 250).
            // На телефоне такая картинка ужимается до нечитаемого, поэтому
            // кладём её в прокручиваемую обёртку и даём открыть во весь экран.
            return '<div class="zd-blockwrap"><img class="zd-img zd-block" ' +
              'src="' + esc(imgSrc(src)) + '" alt="" data-zoom="1"></div>';
          }).join('') + '<div class="zd-zoomhint">' + esc(t.zoom) + '</div>'
      ...
      '<div class="zd-nav">' +
      (state.i > 0 ? '<button class="zd-btn zd-ghost" id="zd-prev">' + esc(t.back) + '</button>' : '') +
      '<button class="zd-btn" id="zd-next">' +
      esc(state.i === total - 1 ? t.finish : t.next) + '</button></div>' +
      '<div class="zd-hint" id="zd-hint"></div></div>');

    root().querySelectorAll('img[data-zoom]').forEach(function (im) {
      im.onclick = function () { lupa(im.src); };
    });
  ...
  function finish() {
    ...
    state.res = {
      ...
      scored: scored,
      percent: overall,
      // Списком, а не строкой: в названиях тем встречаются точки с запятой
      // («There is / There are; some / any / no»), склеенную строку потом
      // не разобрать. И с разбивкой «верно из всего» — в таблице колонка
      // «Слабые темы» заполнена именно так: «Геометрия 0/2; Логика 1/4».
      topics: rows.map(function (r) {
        return { name: r.name, right: r.right, total: r.total, pct: r.pct };
      }),
      // сколько ушло на тест: в таблице под это есть колонка «Минут»
      minutes: state.started
        ? Math.round((Date.now() - state.started) / 6000) / 10
        : '',
  ...
  if (!root()) return;                        // на этой странице теста нет
